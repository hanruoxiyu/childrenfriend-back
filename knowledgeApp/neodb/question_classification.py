...
class Question_classify():
	...
	def read_train_data(self):
		train_x = []
		train_y = []
		file_list = getfilelist("./static/data/question/")
		# 遍历所有文件
	...
	def train_model_NB(self):
		...
		self.tv = TfidfVectorizer()#进行文本转化为特征

		train_data = self.tv.fit_transform(X_train).toarray()
		clf = MultinomialNB(alpha=0.01)#初始化朴素贝叶斯模型
		clf.fit(train_data, y_train)#训练集合上进行训练，估计参数
		# 模型评估
		#print("准确率:", MultinomialNB().score(X_train, y_train))
		return clf
